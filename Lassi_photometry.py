...
sci_data = fits.getdata(r"WASP-12b_example_uncalibrated_images\uncalibrated\WASP-12b_00040.fits").astype(float)
bias_data = fits.getdata(r"WASP-12b_example_raw_biases\bias_00100.fits").astype(float)
dark_data = fits.getdata(r"WASP-12b_example_raw_darks\dark_00150.fits").astype(float)
flat_data = fits.getdata(r"WASP-12b_example_raw_flats\flat_r_00002.fits").astype(float)
...
plt.title("Calibrated Science Image with Detected Stars")
plt.xlabel("X (pixels)")
plt.ylabel("Y (pixels)")
plt.figure(figsize=(7,7))
...
#plt.scatter(sources['xcentroid'], sources['ycentroid'],
#            s=40, facecolors='none', edgecolors='r')
plt.title("Uncalibrated Science Image with Detected Stars")
plt.xlabel("X (pixels)")
plt.ylabel("Y (pixels)")
plt.show()
